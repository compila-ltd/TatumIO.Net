...
namespace TatumIO.Net.Operations
{
    public interface IVirtualAccountOperations
    {
        ...
        Task<TatumBaseResponse> GetAccountAddressesList(string accountId);
    }
    ...
    internal class VirtualAccountOperations : IVirtualAccountOperations
    {
        ...
        /*public async Task<TatumBaseResponse> CreateMultipleDepositAddresses(string accountId, long? index, int count)
		{
            var response = await OffchainEndpoint.CreateMultipleDepositAddresses(accountId, index, count);
            if (response.IsSuccessful)
                return new TatumOkResponse<VirtualAccountAddress>(response.Data ?? throw new Exception(response.ErrorMessage));
            return new TatumErrorResponse(response.ErrorMessage ?? "Error");
        }*/
        ...
        public async Task<TatumBaseResponse> GetAccountAddressesList(string accountId)
        {
            var response = await OffchainEndpoint.GetAccountAddressesList(accountId);
            if (response.IsSuccessful)
                return new TatumOkResponse<VirtualAccountAddressesList>(response.Data ?? throw new Exception(response.ErrorMessage));
            return new TatumErrorResponse(response.ErrorMessage ?? "Error");
        }
